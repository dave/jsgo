...
GopherJS is an amazing tool, but I've always been frustrated by the size of the output. All the packages 
in the dependency tree (including the standard library) are compiled to a single JS file. This can cause 
the resultant file to be several megabytes. 
...
Visit `https://compile.jsgo.io/<path>` to compile or re-compile your package (if your Go package path 
starts `github.com`, you can omit that). Here's a very simple [hello world](https://github.com/dave/jstest/blob/master/main.go) - just click Compile: https://compile.jsgo.io/dave/jstest

After it's finished, you'll be shown a link to a page on `jsgo.io`: https://jsgo.io/dave/jstest. The 
compile page will also give you a link to a single JS file on `pkg.jsgo.io` - this is the 'loader' for 
your package. Add this in a `<script>` tag on your site and it will download all the dependencies 
and execute your package.

### Production ready?

Ths compile server should be considered in beta right now... Please add issues if it's having trouble 
compiling your project. The package serving framework (everything on `pkg.jsgo.io`) should be considered 
relatively production-ready - it's just static JS files in a Google Storage bucket behind a Cloudflare 
CDN so there's very little that can go wrong.

If you'd like to chat more about the project, feel free to file an issue, email me or post in the #gopherjs 
